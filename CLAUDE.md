...
-- Knowledge dependency graph (auto-built from TF-IDF similarity)
CREATE TABLE snippet_dependencies (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  from_id UUID REFERENCES snippets(id) ON DELETE CASCADE NOT NULL,
  to_id UUID REFERENCES snippets(id) ON DELETE CASCADE NOT NULL,
  user_id UUID REFERENCES users(id) ON DELETE CASCADE NOT NULL,
...
CREATE INDEX idx_snippet_deps_from ON snippet_dependencies(from_id);
CREATE INDEX idx_snippet_deps_to ON snippet_dependencies(to_id);
CREATE INDEX idx_snippet_deps_user ON snippet_dependencies(user_id);
```
...
### 7. Knowledge Dependency Graph (graph.py)
- **Purpose:** Automatically detect which snippets are prerequisites of others and build a learning dependency map
- **Input:** list of snippets with {id, title, code, description, tags, created_at}
- **Output:** list of directed edges {from_id, to_id, confidence} — from_id is the prerequisite
- **Logic:**
  - Sort snippets by created_at (earlier-saved = likely prerequisite)
  - Vectorize all snippet text using TFIDFEngine (reuse tfidf.py)
  - For each pair (i, j) where i was saved before j:
    - Compute cosine_similarity(vector_i, vector_j)
    - If similarity >= threshold (default 0.25): create edge from i → j
  - Return all edges sorted by confidence descending
- **Key insight:** No new math — reuses existing TF-IDF + cosine similarity. Graph logic is the novelty.
- **Threshold:** 0.25 (low enough to catch conceptually related snippets, high enough to avoid noise)
- **Technical moat:** Prerequisite-aware SM-2 scheduling — before reviewing snippet B, the system checks if its prerequisite A was recently reviewed. No existing SR tool does this.

### 8. Language Auto-Detection (language_detector.py)
- **Purpose:** Automatically detect the programming language of a pasted code snippet and update the language select in the snippet editor
- **Input:** raw code string
- **Output:** `{language: str, confidence: float (0–1), scores: dict[lang → float]}`
- **Logic:**
  - Define `LANGUAGE_SIGNATURES` — per-language keyword list + regex patterns + weights for 9 languages (TypeScript, JavaScript, Python, CSS, HTML, JSON, Markdown, Bash, SQL)
  - For each candidate language:
    - keyword_score = (unique keyword hits / total keywords) × keyword_weight
    - pattern_score = (unique regex pattern hits / total patterns) × pattern_weight
    - total_score = keyword_score + pattern_score
  - confidence = total_score / max_possible_score (keyword_weight + pattern_weight)
  - Return language with highest score if confidence ≥ 0.08, else return "other"
  - TypeScript vs JavaScript handled by weighting TS-specific type annotations (`: string`, `interface`, generics) 2–4× higher than shared JS keywords — so TS code outscores JS even though TS is a superset
- **Frontend integration:** 400ms debounce on code editor change (min 30 chars). If user hasn't manually changed the dropdown → auto-apply silently + show "Auto" badge. If user already chose a language → show "Detected: X — Switch?" suggestion inline.
- **Tests:** 28 tests, all passing — covers all 9 languages plus edge cases (empty, very short, TS > JS for typed code, JS > TS for CommonJS)
- **Algorithm file:** `apps/api/app/algorithms/language_detector.py`
- **Reference:** Cavnar, W.B. & Trenkle, J.M. (1994). N-gram-based text categorization (inspiration for per-language signature scoring)
...
POST /api/graph/build
  Body: { snippets: list[{id, title, code, description, tags, created_at}], similarity_threshold?: float }
  Returns: { edges: list[{from_id, to_id, confidence}] }

POST /api/detect-language
  Body: { code: str }
  Returns: { language: str, confidence: float, scores: dict[str, float] }
...
- [x] **Day 27 — Tue, May 18:** Build Snippet Detail page (`/snippets/[id]`). Full view with syntax highlighting. SM-2 state display. Edit and delete functionality. Commit: `feat: build snippet detail page with edit and delete`

- [x] **Day 28 — Wed, May 19:** Build Dashboard — stat cards: total snippets, review streak, due today, retention rate. Commit: `feat: build dashboard stat cards`

- [x] **Day 29 — Thu, May 20:** Build Dashboard — cluster section. Call `/api/clusters`. Clickable cluster cards with label, count, snippet previews. Expand on click. Commit: `feat: build dashboard cluster view with K-means`

- [x] **Day 30 — Fri, May 21:** Build Dashboard — review heatmap. GitHub-style activity graph from review_logs grouped by date. Last 12 weeks. Commit: `feat: build review activity heatmap`

- [x] **Day 31 — Sat, May 22:** Build onboarding page (`/onboarding`). Language preferences. First-login only. "Save your first snippet" CTA. Commit: `feat: build onboarding page for new users`

#### Week 7 (May 24–29): Email + Thread Generator + Landing + Polish

- [x] **Day 32 — Mon, May 24:** Set up Resend email. Daily cron (Supabase Edge Function or FastAPI scheduled task) at 9 AM. Query users with due snippets. Send email with "Review now" button linking to /review. Commit: `feat: implement daily email review reminder`

- [x] **Day 33 — Tue, May 25:** Build Thread Draft feature. "Generate thread" button on cluster view. Call `/api/summarize`. Editable thread points with copy-all and copy-individual. Commit: `feat: build X thread draft generator`

- [x] **Day 34 — Wed, May 26:** Build Landing page (`/`). Hero, how-it-works (3 steps), features section, footer. Clean professional design. Commit: `feat: build landing page`

- [x] **Day 35 — Thu, May 27:** Build Settings page (`/settings`). Preferences: languages, reminder time, email on/off. Export data as JSON. Delete account. Commit: `feat: build settings page`

- [x] **Day 36 — Fri, May 28:** Mobile responsive polish. Test all pages at 375px. Sidebar → hamburger. Cards stack. Code scrolls horizontally. Commit: `style: mobile responsive fixes`

- [x] **Day 37 — Sat, May 29:** Full end-to-end test of every feature. Save 10 snippets, review, search, clusters, thread draft, email. Fix all bugs. Commit: `fix: comprehensive end-to-end testing and fixes`

- [x] **Day 37b — Sprint 3 Review:** Merge all `sprint-3/*` branches into `dev`. Full test. Merge `dev` into `main`. Tag `v0.3.0`. App is now feature-complete. Sprint retrospective.
...
- [x] **Day 38 — Mon, May 31:** Write unit tests for frontend components using Vitest. Test: snippet editor form validation, review card state transitions, search result rendering, stat card calculations. Commit: `test: add frontend component unit tests`

- [x] **Day 39 — Tue, Jun 1:** Write system tests — complete user flows: sign up → onboarding → first snippet → first review, save duplicate → warning, search finds results, clusters update after saving. Commit: `test: add system integration tests`

- [x] **Day 40 — Wed, Jun 2:** Edge case testing: empty states (no snippets, no reviews, no clusters), very long code, special characters, 0 due snippets, single snippet clustering, search with no results. Commit: `fix: handle edge cases and empty states`

- [x] **Day 41 (extra) — Language Auto-Detection feature:** Implemented Algorithm 8 — `language_detector.py` (keyword frequency scoring, 9 languages, 28 tests passing). `POST /api/detect-language` FastAPI endpoint. Frontend: 400ms debounced detection in snippet editor, auto-applies language when user hasn't manually selected, shows "Detected: X — Switch?" suggestion otherwise. "Auto" pill badge on the language select. Branch: `sprint-4/feat-language-detector`. Commits: `feat: implement keyword frequency scoring language detection algorithm`, `fix: resolve hydration mismatch in snippet editor language row`

- [ ] **Day 41 — Thu, Jun 3:** Implement Knowledge Dependency Graph feature. Backend: `algorithms/graph.py` (TF-IDF + cosine similarity → DAG edges), `routes/graph.py` (POST /api/graph/build), Pydantic schemas, `tests/test_graph.py`. Frontend: Supabase `snippet_dependencies` table migration, `use-graph.ts` hook, "Builds on / Needed by" sections on snippet detail page, rebuild graph on new snippet save. Branch: `sprint-4/feat-dependency-graph`. Commit: `feat: implement knowledge dependency graph with prerequisite-aware scheduling`
...
**Current Phase:** Phase 4 — Testing + Real Usage
**Current Day:** Day 41 (Thu, Jun 3, 2026)
**Last Updated:** Thu, Jun 3, 2026
**Total Progress:** 42/68 tasks completed (Sprint 1 complete ✓ tagged v0.1.0 | Sprint 2 complete ✓ tagged v0.2.0 | Sprint 3 complete ✓ tagged v0.3.0 — app is feature-complete | Sprint 4 in progress)
**Extra feature added:** Algorithm 8 — Language Auto-Detection (`language_detector.py`) implemented and integrated into the snippet editor. Branch `sprint-4/feat-language-detector` merged to dev and main.
