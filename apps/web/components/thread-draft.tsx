  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 pt-3 pb-1">
        <span className="font-mono text-[10px] font-semibold text-muted-foreground/60 tabular-nums select-none">
      ...
      {text.length > 220 && (
        <div
          className={cn(
            "px-4 pb-2 text-right font-mono text-[10px] tabular-nums",
            charColor(text.length)
          )}
        >
  ...
  return (
    <div className="space-y-3">
      {[80, 120, 100, 90].map((w, i) => (
        ...
        >
          <div className="mb-3 h-2.5 w-6 rounded-full bg-muted" />
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-red-500/10">
        ...
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M10 6v5m0 3h.01M4.07 16h11.86A2 2 0 0 0 17.74 13L11.87 3a2.25 2.25 0 0 0-3.74 0L2.26 13a2 2 0 0 0 1.81 3Z"
          />
        </svg>
      </div>
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        Make sure the algorithm API is running
      </p>
      <button
        type="button"
        onClick={onRetry}
        className="mt-4 rounded-lg bg-muted px-4 py-1.5 text-xs font-medium text-foreground transition-colors duration-150 hover:bg-accent"
      >
        Try again
      </button>
    </div>
...
export function ThreadDraft({
  ...
  async function copyAll() {
    const text = drafts.map((d, i) => `${i + 1}/ ${d}`).join("\n\n")
    await navigator.clipboard.writeText(text)
    setAllCopied(true)
      ...
      >
        ...
        <div className="flex items-center gap-2 border-b border-border px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            <DialogTitle className="truncate font-heading text-sm font-semibold text-foreground capitalize">
