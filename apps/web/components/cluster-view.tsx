...
const ACCENTS = [
  ...
  {
    dot: "bg-pulse",
    pill: "bg-pulse/10 text-pulse",
    border: "hover:border-pulse/25",
  },
  {
    dot: "bg-blue-400",
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="flex w-full items-start gap-3 p-5 text-left transition-transform duration-100 active:scale-[0.99]"
      >
        <span
          className={cn("mt-1.25 h-2 w-2 shrink-0 rounded-full", accent.dot)}
        />

        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2">
            <h3 className="truncate font-heading text-sm font-semibold text-foreground capitalize">
              {cluster.label}
            </h3>
  ...
  return (
    <div className="animate-pulse rounded-2xl border border-border bg-card p-5">
      <div className="flex items-start gap-3">
        <div className="mt-1.25 h-2 w-2 shrink-0 rounded-full bg-muted" />
...
export function ClusterView({
  ...
  const [activeCluster, setActiveCluster] =
    useState<ClusterWithSnippets | null>(null)
  const {
    data: threadPoints,
    isPending,
    isError,
    ...
    return (
      <div className="rounded-2xl border border-dashed border-border/60 bg-card/40 px-6 py-10 text-center">
        ...
        <p className="text-sm font-medium text-foreground">
          3+ snippets needed for clustering
        </p>
